...
func docsFeatureItem(icon, title, desc string) g.Node {
	return Div(Class("flex gap-4 items-start"),
		...
		Div(
			H3(Class("text-base font-semibold text-slate-100 mb-1"), g.Text(title)),
			P(Class("text-sm text-slate-400 leading-relaxed"), g.Text(desc)),
		),
	)
}
...
func DocsContent() g.Node {
	return Main(Class("container mx-auto mt-10 mb-20 px-4 max-w-3xl"),
		...
		Div(Class("mb-10"),
			...
			P(Class("text-slate-400 text-lg"), g.Text("Learn how to use bbscope.com to track bug bounty program scopes.")),
		),
		...
		Section(Class("bg-slate-900/30 border border-slate-800/50 rounded-2xl shadow-xl shadow-black/10 p-6 md:p-8 mb-6"),
			...
			P(Class("text-slate-400 leading-relaxed"), g.Text("This platform aggregates public bug bounty program scopes from HackerOne, Bugcrowd, Intigriti, and YesWeHack. All data is fetched automatically using the bbscope CLI tool.")),
		),
		...
		Section(Class("bg-slate-900/30 border border-slate-800/50 rounded-2xl shadow-xl shadow-black/10 p-6 md:p-8 mb-6"),
			...
			Ol(Class("space-y-3 text-slate-400 list-decimal list-inside"),
		...
		Section(Class("bg-slate-900/30 border border-slate-800/50 rounded-2xl shadow-xl shadow-black/10 p-6 md:p-8"),
			...
			P(Class("text-slate-400 leading-relaxed mb-4"), g.Text("bbscope is an open-source project. Contributions are welcome!")),
			A(Href("https://github.com/sw33tLie/bbscope"), Target("_blank"), Rel("noopener noreferrer"),
				Class("inline-flex items-center gap-2 px-4 py-2 bg-slate-800/50 border border-slate-700/50 rounded-lg text-sm text-slate-300 hover:text-cyan-400 hover:border-slate-600 transition-all duration-200"),
